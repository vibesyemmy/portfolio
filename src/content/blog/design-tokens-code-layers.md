...
I'm building a design system in Figma right now. Not designing screens. Building the system that builds screens. Ninety-three variables, every color bound to a semantic token. Ten text styles. Eleven components. The pipeline is deliberate: Figma variable → design token → CSS custom property → component code. Months of work, all organized around the idea that the token is the source of truth and the code is the output.

Then Config 2026 happened.

Dylan Field walked on stage and said: "Code is not the opposite of design. Code is material for design." Then he showed Code Layers: any design frame becomes interactive code with a prompt or a click. Duplicate it. Iterate on it. Comment on it. Treat it like any other layer on your canvas. It's in closed beta for now, but the direction is unambiguous.
...
It's a layer type. A first-class canvas primitive, sitting alongside frames, text, and vectors. You can extract design frames from a GitHub repository. You can convert any design layer into working component code with a prompt or a click. You can go the other direction: pull interaction flows out of code and inspect them as design layers. It's bidirectional. The handoff pipeline doesn't get faster. It dissolves.
...
A Code Layer gives you *this button* as a component. The exact element, on the exact screen, in the exact state you designed. Beautiful. Precise. And also: a point solution.
...
Code Layers solve the translation problem. Tokens solve the consistency problem. They are not competing. They operate at different altitudes. A Code Layer without a token system is a beautiful one-off: precise, fast, unrepeatable at scale. A token system without Code Layers is a promise you still have to verify manually.

The real power is when they meet: a Code Layer that inherits from your token system. Generate the code, and the button already knows it's `bg-primary`, not `bg-blue-500`. Already uses `spacing-3`, not `12px`. The token was there before the code existed. The code just surfaced it.

## Figma's playbook

There's a pattern here. Figma has been eating the layers between design and production for a decade.

In 2016, prototyping moved into Figma. Suddenly you didn't need InVision or Marvel (standalone prototyping tools) to show a flow. The prototype lived on the same canvas as the design.

In 2019, developer handoff moved in. Inspect mode, code snippets, measurements. Tools like Zeplin and Abstract, which existed solely to bridge design files and engineering specs, became optional. Each time, the tool absorbed what sat between the design file and the shipped product.
...
Type B understands component architecture. They know why a token system has tiers: primitive tokens feed semantic tokens feed component tokens. They can read the output Code Layers generates and spot when a color is hardcoded instead of inherited. They don't need to write production code, but they can have a real conversation about component composition with an engineer. Not a handoff. A conversation.

Type A isn't bad at their job. They're skilled, precise, and their frames ship. But the part of the job they're best at: translating designs into specifications for engineers, is exactly what Code Layers automates. That's uncomfortable to hear, but it's true.

Type B has been investing in the layer above. They think in systems, not screens. A button isn't a rectangle with a label. It's a set of rules: what changes, what doesn't, what inherits, what overrides. Code Layers doesn't threaten that skill set. It amplifies it.

The question isn't which type you are today. It's which type you're building toward.

## The invisible code

Here's the thing Figma's framing quietly obscures: the token was always code.

A CSS custom property is code. A JSON token file is code. A design token specification, whether it's the W3C format, Style Dictionary, or Tokens Studio: is code. It's declarative code: it says what should be true. It doesn't execute. But it's code all the same.

What Code Layers introduced isn't *code* entering the design canvas. It's *imperative* code entering the design canvas. Code that renders. Code that responds to state. The declarative code, the tokens, was already there. It was just invisible to anyone who didn't think of it as code.
...
The token was always code. Code Layers just made it visible.
