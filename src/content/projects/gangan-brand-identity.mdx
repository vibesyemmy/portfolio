...
The only systematic color lives in the status palette — green for success, amber for warnings, red for errors — each paired with a soft tint for backgrounds and surfaces. These aren't brand accents; they're functional signals, used only where the interface needs to communicate state. Everything else is grayscale.
